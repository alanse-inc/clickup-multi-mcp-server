...
export class WorkspaceService extends BaseClickUpService {
   ...
   */
  async createSpace(data: CreateSpaceData): Promise<ClickUpSpace> {
    try {
      if (!data.name) {
        throw new ClickUpServiceError(
      ...
      logger.info(`Creating space: ${data.name}`);

      const result = await this.makeRequest(async () => {
        const response = await this.client.post(`/team/${this.teamId}/space`, data);
        return response.data;
      });

      // Clear cached hierarchy since we added a new space
      this.clearWorkspaceHierarchy();

      logger.info(`Space created successfully: ${result.name} (${result.id})`);
      return result;
    } catch (error) {
      throw this.handleError(error, 'Failed to create space');
    }
  }
   ...
   * @param data - Space update data
   * @returns - Promise resolving to the updated space
   */
  async updateSpace(spaceId: string, data: UpdateSpaceData): Promise<ClickUpSpace> {
    try {
      if (!spaceId) {
        throw new ClickUpServiceError(
          'Space ID is required',
      ...
      const result = await this.makeRequest(async () => {
        const response = await this.client.put(`/space/${spaceId}`, data);
        return response.data;
      });

      // Clear cached hierarchy since we modified a space
      this.clearWorkspaceHierarchy();

      logger.info(`Space updated successfully: ${result.name} (${result.id})`);
      return result;
    } catch (error) {
  ...
  async deleteSpace(spaceId: string): Promise<void> {
    try {
      ...
      // Clear cached hierarchy since we removed a space
      this.clearWorkspaceHierarchy();
