...
export class TaskService extends TaskServiceCore {
  ...
  public readonly dependencies: TaskServiceDependencies;
  ...
  constructor(apiKey: string, teamId: string, baseUrl?: string, workspaceService?: WorkspaceService) {
    ...
    this.dependencies = new TaskServiceDependencies(this);
  }
  ...
  async addDependency(taskId: string, data: AddDependencyData): Promise<TaskDependency> {
    return this.dependencies.addDependency(taskId, data);
  }

  async deleteDependency(taskId: string, dependsOn?: string, dependencyOf?: string): Promise<void> {
    return this.dependencies.deleteDependency(taskId, dependsOn, dependencyOf);
  }

  async addTaskLink(taskId: string, linksTo: string): Promise<TaskLink> {
    return this.dependencies.addTaskLink(taskId, linksTo);
  }

  async deleteTaskLink(taskId: string, linksTo: string): Promise<void> {
    return this.dependencies.deleteTaskLink(taskId, linksTo);
  }
}